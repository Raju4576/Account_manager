...
const Updaterecord = ({ onCancel, id }) => {
  ...
  useEffect(() => {
    fetch(`${ipaddress}/findrecord/${id}`)
      .then(res => res.json())
      .then(data => {
        if (data.data) {
          const { t_date, t_type, t_amt, t_particular } = data.data;
          setDate(t_date);
          setType(t_type);
          setAmt(t_amt.toString()); // Assuming t_amt is numeric
          setPert(t_particular);
        }
      })
      .catch(error => {
  ...
  const Add = async () => {
    if (validate()) {
      const response = await fetch(`${ipaddress}/tupdate/${id}`, {
        method: 'POST',
        headers: {
    ...
    <View style={styles.maincont}>
      <View style={styles.dialog}>
        <View style={styles.text}>
          <Text style={{ color: 'white', fontWeight: 'bold', textAlign: 'center' }}>Update Transaction</Text>
        </View>
        <View style={{ flexDirection: 'column' }}>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-around', marginTop: '2%' }}>
            ...
            <Pressable style={styles.btn2} onPress={Add}>
              <Text style={{ color: 'white', fontSize: 15, fontWeight: "bold", textAlign: 'center' }}>Update</Text>
            </Pressable>
          </View>
        </View>
      </View>
